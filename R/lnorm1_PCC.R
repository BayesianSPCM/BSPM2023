# Function for running PCC LogNormal with mean unknown

lnorm1_PCC <- function( data = NULL, historical_data = NULL,
                        ...
                        main = "PCC LogNormal with unknown mean" )
{
  ...
  if ( !is.null(historical_data) ){
    ...
    mu0_PowerP <- ( sdl^2*mu0 + sd0^2*alpha_0*sum( log(historical_data) ) ) / ( sdl^2 + sd0^2*alpha_0*N_historicaldata )
  ...
  mu0_Post <- ( sdl^2*mu0 + sd0^2*datalogSum )/( sdl^2 + sd0^2*(1:N) )
  sd_Post <- sqrt( ( sdl^2*sd0^2 ) / ( sdl^2 + sd0^2*(1:N) ) )


  # Control limits
  if (!FIR) { CL <- t( mapply( function(MP, SDT, FD = FAR) {
    hd <- lnorm_HD( cover = 1-FD, mulog = MP, sdlog = SDT )
  ...
  if ( PriorPosterior_PLOT | pdf_report ) {
    ...
    PrPostPLOT <-
      ...
      ggtitle(expression(atop("PCC LogNormal likelihood - unknown mean"~theta, "Prior/Posterior distribution"))) +
      {if(mu0 == 0 & sd0 == 10^6) {
