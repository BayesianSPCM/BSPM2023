...
norm2_PCC <- function( data = NULL, historical_data = NULL, ml = NULL,
                       ...
                       main = "PCC Normal with unknown variance" )
{
  ...
  if (!FIR) { CL <- t( mapply( function(MP, AP, BP, FD=FAR) {
    hd <- t_HD( cover = 1-FD, df = 2*ifelse(AP > 0, AP, NA), mu = MP, sdv = sqrt( BP/AP ) )
  ...
  if ( PriorPosterior_PLOT | pdf_report ) {
    ...
    PP <- data.frame( x = c(
                      ifelse( a0 == 0 & b0 == 0, qinvgamma(1 - .9999, shape = a0_Post[N], rate = b0_Post[N]),
                              min( qinvgamma(1 - .9999, shape = a0, rate = b0), qinvgamma(1 - .9999, shape = a0_Post[N], rate = b0_Post[N]) ) ),
                      ifelse( a0 == 0 & b0 == 0, qinvgamma(.9999, shape = a0_Post[N], rate = b0_Post[N]),
    ...
    PrPostPLOT <-
      ...
      ggtitle(expression(atop("PCC Normal likelihood - unknown variance"~theta^2, "Prior/Posterior distribution"))) +
      {if(a0 == 0 & b0 == 0) {
