# Function for running PCC LogNormal with variance unknown

lnorm2_PCC <- function( data = NULL, historical_data = NULL,
                        ...
                        main = "PCC LogNormal with unknown variance" )
{
  ...
  if ( !is.null(historical_data) ){
    ...
    b0_PowerP  <- b0 + alpha_0*sum( (log(historical_data) - ml )^2 )/2
  ...
  datalogSum2 <- cumsum( (log(data) - ml )^2 )[seq( 1, length(data) )]

  # Posterior distribution parameters
  a0_Post <- a0 + (1:N)/2
  b0_Post <- b0 + datalogSum2/2



  # Control limits
  if (!FIR) { CL <- t( mapply( function(MP, AP, BP, FD=FAR) {
    hd <- lt_HD( cover = 1-FD, df = 2*ifelse(AP > 0, AP, NA), mulog = MP, sdlog = sqrt( BP/AP ) )
    return( c( hd$lower.bound, hd$upper.bound ) )
  }, MP = ml, AP = a0_Post, BP = b0_Post) )
  } else { CL <- t( mapply( function(MP, AP, BP, FD) {
    hd <- lt_HD( cover = 1-FD, df = 2*ifelse(AP > 0, AP, NA), mulog = MP, sdlog = sqrt( BP/AP ) )
    return( c( hd$lower.bound, hd$upper.bound ) )
  ...
  if ( PriorPosterior_PLOT | pdf_report ) {
    ...
    PrPostPLOT <-
      ...
      {if(a0 == 0 & b0 == 0) { stat_function(fun = function(x) { 1/(x + .275)^2 }, aes(colour = "Prior", linetype = "Prior"), size = 1)
      ...
      ggtitle(expression(atop("PCC LogNormal likelihood - unknown variance"~theta^2, "Prior/Posterior distribution"))) +
      {if(a0 == 0 & b0 == 0) {
