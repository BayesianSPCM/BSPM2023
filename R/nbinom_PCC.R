...
nbinom_PCC <- function( data = NULL, historical_data = NULL,
                        ...
                        main = "PCC Negative Binomial with unknown probability" )
{
  ...
  # Control limits
  if (!FIR) { CL <- t( mapply( function(RL, A0, B0, FD = FAR) {
    hd <- betanbinom_HM( cover = 1-FD, r = RL, a = A0, b = B0 )
    return( c( hd$lower.bound, hd$upper.bound ) )
  }, RL = rl, A0 = a0_Post, B0 = b0_Post ) )
  } else { CL <- t( mapply( function(RL, A0, B0, FD) {
    hd <- betanbinom_HM( cover = 1-FD, r = RL, a = A0, b = B0 )
    return( c( hd$lower.bound, hd$upper.bound ) )
  ...
  if ( PriorPosterior_PLOT | pdf_report ) {
    ...
    PrPostPLOT <-
      ...
      {if(a0 == 0 & b0 == 1/2) { stat_function(fun = function(x) { ( 1/( x * sqrt( 1-x ) ) ) / (dbeta( .975 * (a0_Post[N]/(a0_Post[N] + b0_Post[N])), shape1 = a0_Post[N], shape2 = b0_Post[N] )) }, aes(colour = "Prior", linetype = "Prior"), size = 1)
      ...
      ggtitle(expression(atop("PCC Negative Binomial likelihood - unknown probability"~theta, "Prior/Posterior distribution"))) +
      {if(a0 == 0 & b0 == 1/2) {
