# Function for running PCC Inverse-Gamma with unknown scale

invgamma_PCC <- function( data = NULL, historical_data = NULL,
                          ...
                          main = "PCC Inverse-Gamma with unknown scale parameter" )
{
  ...
  # Likelihood al input (i) more than one value for parameters (ii) non-numeric input
  if ( is.null(al) ) {
    stop("Likelihood shape parameter 'al' has not been defined")
  } else {
    if ( length(unlist(al))>1 ) { message("More than one value for 'al', the first one will only be used")
      if ( !is.numeric(al) | al<=0 ) { stop("Invalid 'al' value") } else { al <- al[1] }
    } else { if ( !is.numeric(al) | al<=0 ) { stop("Invalid 'al' value") } }
  }
  ...
  if ( !is.null(historical_data) ){
    ...
    c0_PowerP <- c0 + al*alpha_0*N_historicaldata
    d0_PowerP  <- d0 + alpha_0*sum( 1/historical_data )
    # Keep similar notation as input
    c0 <- c0_PowerP ; d0 <- d0_PowerP
  }

  ### PCC implementation
  # Sum of observations
  data1Sum <- cumsum( 1/data )[seq( 1, length(data) )]
  ...
  # Control limits
  if (!FIR) { CL <- t( mapply( function(A0, C0, D0, FD = FAR) {
    hd <- gb2_HD( cover = 1-FD, a = A0, c = C0, d = D0 )
    return( c( hd$lower.bound, hd$upper.bound ) )
  }, A0 = al, C0 = c0_Post, D0 = d0_Post ) )
  } else { CL <- t( mapply( function(A0, C0, D0, FD) {
    hd <- gb2_HD( cover = 1-FD, a = A0, c = C0, d = D0 )
    return( c( hd$lower.bound, hd$upper.bound ) )
  }, A0 = al, C0 = c0_Post, D0 = d0_Post, FD = FAR ) )
  }
  ...
  if ( PriorPosterior_PLOT | pdf_report ) {
    ...
    PP <- data.frame(x = c(
        ifelse( c0 == 0 & d0 == 0, .15,
                min( qgamma(1 - .999, shape = c0, rate = d0), qgamma(1 - .999, shape = c0_Post[N], rate = d0_Post[N]) ) ),
        ifelse( c0 == 0 & d0 == 0, qgamma(.999, shape = c0_Post[N], rate = d0_Post[N]) + c0_Post[N]/d0_Post[N],
                if (c0 > d0) { min( qgamma(.999, shape = c0, rate = d0), qgamma(.999, shape = c0_Post[N], rate = d0_Post[N]) ) + c0/d0
                } else { max( qgamma(.999, shape = c0, rate = d0), qgamma(.999, shape = c0_Post[N], rate = d0_Post[N]) ) + c0/d0 } )
                          )
                      )

    PrPostPLOT <-
      ggplot( PP, aes(x = x) ) +
      stat_function(fun = dgamma, args = list(shape = c0_Post[N], rate = d0_Post[N]),
                             aes(colour = "Posterior", linetype = "Posterior"), size = 1) +
      {if(c0 == 0 & d0 == 0) { stat_function(fun = function(x) { 1/(x + abs(log(c0_Post[N]/d0_Post[N]))) }, aes(colour = "Prior", linetype = "Prior"), size = 1)
      ...
      scale_colour_manual(values = c("#3CB371", "#FF4500"),
                                   labels = c( bquote("Prior: Gamma(" ~ theta ~ "|" ~ .(round(c0, digits = 1)) ~ ", " ~ .(round(d0, digits = 1)) ~ ")" ),
                                               bquote("Posterior: Gamma(" ~ theta ~ "|" ~ .(round(c0_Post[N], digits = 1)) ~ ", " ~ .(round(d0_Post[N], digits = 1)) ~ ")" )),
                                   guide = guide_legend(override.aes = list( color = c("#FF4500", "#3CB371"),
                                                                             linetype = c("dashed", "solid"),
                                                                             size = c(.5, .5)), title = NULL)) +
      ggtitle(expression(atop("PCC Inverse-Gamma likelihood - unknown rate"~theta, "Prior/Posterior distribution"))) +
      {if(c0 == 0 & d0 == 0) {
        geom_point( aes(x = c0_Post[N]/d0_Post[N], y = 0), color = "#3CB371",  show.legend = FALSE, shape = 4, size = 3, stroke = 1.5, na.rm = TRUE )
      } else { geom_point( aes(x = c(c0/d0, c0_Post[N]/d0_Post[N]), y = c(0, 0)), color = c("#FF4500", "#3CB371"),  show.legend = FALSE, shape = 4, size = 3, stroke = 1.5, na.rm = TRUE ) } } +
      {if(c0 == 0 & d0 == 0) {
        annotate("text", x = c0_Post[N]/d0_Post[N], y = 0, label = paste(expression(mu[post])), color = "#3CB371", size = 6, parse = TRUE, vjust = 1.25)
      } else { annotate("text", x = c(c0/d0, c0_Post[N]/d0_Post[N]), y = c(0, 0), label = paste(expression(mu[prior], mu[post])),
  ...
  if ( PCC_PLOT | pdf_report ) {
    ...
    if ( !is.null(historical_data) ) {
      ...
      PCC_PlotSummary <- cbind( Indices = c(-N_historicaldata:(-1), 1:N),
                                TypeOfdata = c(rep("Historical", times = N_historicaldata), rep("Current", times = N)),
                                PCC_summary_historicaldata )
      PCC_historical <- ggplot( PCC_PlotSummary, aes(PCC_PlotSummary[, "Indices"], PCC_PlotSummary[, "data"]) ) +
        geom_line( aes(x = PCC_PlotSummary[, "Indices"], y = PCC_PlotSummary[, "data"], linetype = as.factor(PCC_PlotSummary[, "TypeOfdata"])), na.rm = TRUE ) +
        geom_segment( aes(x = 0, y = min(PCC_PlotSummary[, "HPrD_LL"], na.rm = TRUE), xend = 0, yend = max(PCC_PlotSummary[, "HPrD_UL"], na.rm = TRUE)) ) +
        geom_line( aes(x = PCC_PlotSummary[, "Indices"], y = PCC_PlotSummary[, "HPrD_UL"]), color = "red", linetype = "solid", size = 1, na.rm = TRUE ) +
        geom_line( aes(x = PCC_PlotSummary[, "Indices"], y = PCC_PlotSummary[, "HPrD_LL"]), color = "red", linetype = "solid", size = 1, na.rm = TRUE ) +
        geom_ribbon( aes(x = PCC_PlotSummary[, "Indices"], ymin = PCC_PlotSummary[, "HPrD_UL"], ymax = PCC_PlotSummary[, "HPrD_LL"], fill = TRUE), alpha = 0.25, show.legend = FALSE ) +
        scale_fill_manual( values = c("TRUE" = "green") ) +
        geom_point( aes(group = PCC_PlotSummary[, "Indices"], shape = as.factor(PCC_PlotSummary[, "TypeOfdata"]), color = as.factor(PCC_PlotSummary[, "Alarms"]), stroke = 1.5),
