# Function for running PCC Gamma with unknown rate parameter

gamma_PCC <- function( data = NULL, historical_data = NULL, al = NULL,
  ...
  if ( !is.null(historical_data) ){
    ...
    d0_PowerP  <- d0 + alpha_0*sum(historical_data)
  ...
  if (!FIR) { CL <- t( mapply( function(A0, C0, D0, FD = FAR) {
    hd <- compgamma_HD( cover = 1-FD, a = A0, c = C0, d = D0 )
  ...
  if ( PriorPosterior_PLOT | pdf_report ) {
    ...
    PrPostPLOT <-
      ...
      ggtitle(expression(atop("PCC Gamma likelihood - unknown rate"~theta, "Prior/Posterior distribution"))) +
      {if(c0 == 0 & d0 == 0) {
